...
class TinnitusNoteController {
    ...
    static async findByPatient(req, res) {
        try {
            ...
            Logger_1.Logger.info('TinnitusNoteController.findByPatient - Solicitud recibida', { id_patient });
    ...
    static async createForPatient(req, res) {
        try {
            const { id_patient } = req.params;
            Logger_1.Logger.info('TinnitusNoteController.createForPatient - Solicitud recibida', {
                id_patient,
                body: req.body,
            });
            const data = dto_1.CreateTinnitusNoteSchema.parse({ ...req.body, id_patient });
            const useCase = new usecases_1.CreateTinnitusNoteUseCase(noteRepository, patientRepository, questionnaireRepository, responseRepository);
            const result = await useCase.execute({
                idPatient: data.id_patient,
                idTinnitusQuestionnaires: data.id_tinnitus_questionnaires,
            ...
            Logger_1.Logger.success('TinnitusNoteController.createForPatient - Nota creada', { id: result.id });
            res.status(201).json(result);
        }
        catch (error) {
            if (error instanceof zod_1.ZodError) {
                Logger_1.Logger.warning('TinnitusNoteController.createForPatient - Error de validación Zod', {
                    errors: error.errors,
                });
                throw new errorHandler_1.ValidationAppError('Datos de entrada inválidos', error.errors);
            }
            const errorMessage = error.message;
            if (errorMessage.includes('no encontrado') || errorMessage.includes('no existe')) {
                Logger_1.Logger.warning('TinnitusNoteController.createForPatient - Entidad no encontrada', {
                    error: errorMessage,
                });
                throw new errorHandler_1.NotFoundAppError(errorMessage);
            }
            Logger_1.Logger.danger('TinnitusNoteController.createForPatient - Error interno', {
                error: errorMessage,
            });
            throw error;
        }
