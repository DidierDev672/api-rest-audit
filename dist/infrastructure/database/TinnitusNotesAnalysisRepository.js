...
exports.TinnitusNotesAnalysisRepository = void 0;
const uuid_1 = require("uuid");
const supabase_1 = require("../database/supabase");
const Logger_1 = require("../logger/Logger");
class TinnitusNotesAnalysisRepository {
    constructor() {
        this.table = 'tinnitus_notes_analysis';
    }
    async create(data) {
        const id = (0, uuid_1.v4)();
        const now = new Date();
        Logger_1.Logger.info('Creando análisis de notas de tinnitus', {
        ...
        const { data: result, error } = await supabase_1.supabase
            ...
            .single();
        if (error) {
            Logger_1.Logger.danger('Error al crear análisis de notas de tinnitus', { error: error.message });
            throw new Error(error.message);
        }
        Logger_1.Logger.success('Análisis de notas de tinnitus creado', { id });
    ...
    async findAll() {
        ...
        if (error) {
            Logger_1.Logger.danger('Error al obtener análisis de notas de tinnitus', { error: error.message });
            throw new Error(error.message);
        }
        Logger_1.Logger.success('Análisis de notas de tinnitus obtenidos', { count: data.length });
    ...
    async findByPatientId(patientId) {
        ...
        const { data, error } = await supabase_1.supabase
            ...
            .order('created_at', { ascending: false });
        if (error) {
            Logger_1.Logger.danger('Error al obtener análisis de notas de tinnitus por paciente', { error: error.message });
            throw new Error(error.message);
        }
        Logger_1.Logger.success('Análisis de notas de tinnitus por paciente obtenidos', { count: data.length, patientId });
        return data.map(this.mapToEntity);
    }
    async findByTinnitusQuestionnaireId(questionnaireId) {
        ...
        const { data, error } = await supabase_1.supabase
            ...
            .order('created_at', { ascending: false });
        if (error) {
            Logger_1.Logger.danger('Error al obtener análisis de notas de tinnitus por cuestionario', { error: error.message });
            throw new Error(error.message);
        }
        Logger_1.Logger.success('Análisis de notas de tinnitus por cuestionario obtenidos', { count: data.length, questionnaireId });
        return data.map(this.mapToEntity);
    }
    async findByTinnitusResponseId(responseId) {
        ...
        const { data, error } = await supabase_1.supabase
            ...
            .order('created_at', { ascending: false });
        if (error) {
            Logger_1.Logger.danger('Error al obtener análisis de notas de tinnitus por respuesta', { error: error.message });
            throw new Error(error.message);
        }
        Logger_1.Logger.success('Análisis de notas de tinnitus por respuesta obtenidos', { count: data.length, responseId });
        return data.map(this.mapToEntity);
    }
    async update(id, data) {
        ...
        if (data.noteCount !== undefined)
            updateData.note_count = data.noteCount;
        if (data.analyzedAt !== undefined)
            updateData.analyzed_at = data.analyzedAt;
        if (data.createdBy !== undefined)
            updateData.created_by = data.createdBy;
        const { data: result, error } = await supabase_1.supabase
            .from(this.table)
            .update(updateData)
            .eq('id', id)
            ...
            .single();
        if (error) {
            Logger_1.Logger.danger('Error al actualizar análisis de notas de tinnitus', { error: error.message });
            throw new Error(error.message);
        }
    ...
    async delete(id) {
        ...
        const { error } = await supabase_1.supabase
            ...
            .eq('id', id);
        if (error) {
            Logger_1.Logger.danger('Error al eliminar análisis de notas de tinnitus', { error: error.message });
            throw new Error(error.message);
        }
...
exports.TinnitusNotesAnalysisRepository = TinnitusNotesAnalysisRepository;
//# sourceMappingURL=TinnitusNotesAnalysisRepository.js.map