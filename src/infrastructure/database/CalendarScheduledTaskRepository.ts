...
} from '../../domain/repositories/ICalendarScheduledTaskRepository';

export class CalendarScheduledTaskRepository implements ICalendarScheduledTaskRepository {
  private readonly table = 'calendar_scheduled_tasks';
  ...
  async findAll(
    filters: CalendarScheduledTaskFilters = {}
  ): Promise<CalendarScheduledTask[]> {
    let query = supabase.from(this.table).select('*').order('scheduled_at', {
      ascending: true,
    });
  ...
  async findById(id: string): Promise<CalendarScheduledTask | null> {
  ...
  async findDuePending(limit = 50): Promise<CalendarScheduledTask[]> {
  ...
  async cancelByCalendarEventId(calendarEventId: string): Promise<void> {
  ...
  private mapToEntity(data: Record<string, unknown>): CalendarScheduledTask {
    return {
      ...
      status: data.status as CalendarScheduledTask['status'],
      channel: data.channel as CalendarScheduledTask['channel'],
      ...
      sentAt: data.sent_at ? new Date(data.sent_at as string) : null,
